...
% \documentclass[aspectratio=11]{beamer}
...
\newcommand{\TinyCap}{\fontsize{7.6}{8.6}\selectfont}
...
\begin{minipage}[t][0.80\textheight]{\linewidth}\vspace{0pt}
  ...
  Bayesian optimization is effective for expensive black-box objectives, but practical BO with flexible neural surrogates still needs a useful uncertainty mechanism.
  \end{cardA}

  \vfill

  \begin{card}{Standard neural baseline}
  MC-Dropout interprets dropout as approximate Bayesian inference: different masks induce different surrogate views and predictive uncertainty.
  \end{card}
...
\begin{minipage}[t][0.80\textheight]{\linewidth}\vspace{0pt}
  ...
  The project treats dropout-mask generation as a \textbf{structured sampling problem} and uses a GFlowNet to bias probability toward masks with higher improvement potential.
  \end{card}
\end{minipage}
\end{columns}
\end{frame}
...
\begin{minipage}[t][0.80\textheight]{\linewidth}\vspace{0pt}
  ...
  For candidate pool $C_t$, each mask proposes
  \[
  x_m=\arg\max_{x\in C_t}\hat f_\theta(x;m).
  \]
...
% ------------------------------------------------
\begin{frame}{Procedure at one BO iteration}
\vspace{-0.55em}
\begin{columns}[T,onlytextwidth]
\column{0.49\textwidth}
\begin{minipage}[t][0.80\textheight]{\linewidth}\vspace{0pt}
  \begin{cardA}{Step-by-step procedure}
  \begin{enumerate}
    \item Train the masked neural surrogate on current dataset $D_t$.
    \item Sample a set of candidate masks.
    \item For each mask, select its preferred point from the candidate pool.
    \item Score that proposal with the proxy reward.
    \item Query the point from the best sampled mask.
  \end{enumerate}
  \end{cardA}

  \vfill

  \begin{card}{What is held fixed}
  Both methods use the same surrogate, BO horizon, candidate-pool size, held-out evaluation protocol, and per-step mask budget.
  \end{card}
\end{minipage}

\column{0.47\textwidth}
\begin{minipage}[t][0.80\textheight]{\linewidth}\vspace{0pt}
  \begin{cardG}{Random baseline}
  \begin{itemize}
    \item sample block-wise masks uniformly
    \item evaluate each sampled mask with the same reward protocol
    \item query the proposal from the best sampled mask
  \end{itemize}
  \end{cardG}

  \vfill

  \begin{cardW}{GFlowNet policy}
  \begin{itemize}
    \item train a sequential policy so mask probability tracks reward
    \item sample masks from that learned policy
    \item again query the proposal from the best sampled mask
  \end{itemize}
  \end{cardW}

  \vfill

  \begin{card}{Main contrast}
  The comparison isolates \textbf{mask-sampling policy} rather than differences in the surrounding BO loop.
  \end{card}
\end{minipage}
\end{columns}
\end{frame}
...
\begin{minipage}[t][0.80\textheight]{\linewidth}\vspace{0pt}
  ...
  \begin{cardG}{Benchmark configurations}
  \centering
  \scriptsize
...
\begin{minipage}[t][0.78\textheight]{\linewidth}\vspace{0pt}
  \begin{card}{Branin (2D)}
  \centering
  \includegraphics[width=\linewidth,height=0.58\textheight,keepaspectratio]{img/branin_regret_comparison.png}
...
\begin{minipage}[t][0.78\textheight]{\linewidth}\vspace{0pt}
  ...
  \centering
  \includegraphics[width=\linewidth,height=0.58\textheight,keepaspectratio]{img/hartmann6_regret_comparison.png}
...
\begin{minipage}[t][0.78\textheight]{\linewidth}\vspace{0pt}
  ...
  \centering
  \includegraphics[width=\linewidth,height=0.58\textheight,keepaspectratio]{img/ackley10_regret_comparison.png}
...
\begin{minipage}[t][0.80\textheight]{\linewidth}\vspace{0pt}
  \begin{cardA}{Best defensible claim}
...
\begin{minipage}[t][0.80\textheight]{\linewidth}\vspace{0pt}
  \begin{card}{Final regret over 5 seeds}
  \centering
  \scriptsize
  \begin{tabular}{lcc}
  \toprule
  Benchmark & Random & GFlowNet \\
  \midrule
  ...
  \vspace{0.45em}
  {\TinyCap Lower regret is better. Ackley-10 is the only benchmark where GFlowNet improves final regret.}
  \end{card}
\end{minipage}
\end{columns}
\end{frame}
...
\begin{minipage}[t][0.78\textheight]{\linewidth}\vspace{0pt}
  ...
  \centering
  \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{img/ackley10_random_diagnostics.png}
...
\begin{minipage}[t][0.78\textheight]{\linewidth}\vspace{0pt}
  ...
  \centering
  \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{img/ackley10_gfn_diagnostics.png}
...
% ------------------------------------------------
\appendix
\begin{frame}{Appendix: later full-rerun comparison with plain random dropout}
\vspace{-0.55em}
\begin{columns}[T,onlytextwidth]
\column{0.50\textwidth}
\begin{minipage}[t][0.80\textheight]{\linewidth}\vspace{0pt}
  \begin{cardW}{How to read this slide}
  This is an additional \textbf{robustness check} from a later \textbf{full rerun} of the v2 pipeline. It asks whether the matched random baseline is stronger than \textbf{plain random dropout} that samples exactly one mask per BO step.
  \end{cardW}

  \vfill

  \begin{cardA}{What changes}
  \begin{itemize}
    \item \textbf{Plain random}: one uniformly sampled mask per BO step
    \item \textbf{Matched random}: best-of-96 random masks under the same proxy evaluation
    \item \textbf{GFN}: learned mask policy under the same later v2 rerun setup
  \end{itemize}
  \end{cardA}

  \vfill

  \begin{cardG}{Takeaway}
  The matched random baseline is not a trivial strawman:
  \begin{itemize}
    \item it clearly improves over plain random on Hartmann-6
    \item it is still weaker than GFlowNet on Ackley-10
    \item on Branin, even plain random remains competitive
  \end{itemize}
  \end{cardG}
\end{minipage}
...
\column{0.46\textwidth}
\begin{minipage}[t][0.80\textheight]{\linewidth}\vspace{0pt}
  \begin{card}{Final regret over 5 seeds (v2 only)}
  \centering
  \scriptsize
  \begin{tabular}{lccc}
  \toprule
  Benchmark & Plain random & Matched random & GFlowNet \\
  \midrule
  Branin (2D) & $2.3177 \pm 1.9155$ & $2.4758 \pm 1.8689$ & $3.0355 \pm 1.4537$ \\
  Hartmann-6 (6D) & $0.9728 \pm 0.5025$ & $0.7294 \pm 0.5219$ & $0.8534 \pm 0.4978$ \\
  Ackley (10D) & $18.8398 \pm 0.3623$ & $19.5119 \pm 0.8592$ & $18.3047 \pm 0.6267$ \\
  \bottomrule
  \end{tabular}

  \vspace{0.45em}
  {\TinyCap Lower regret is better. These numbers come from a later full rerun of the v2 pipeline and isolate the effect of upgrading from plain random masks to a stronger best-of-K random baseline.}
  \end{card}
\end{minipage}
\end{columns}
\end{frame}
